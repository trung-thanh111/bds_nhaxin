            ...
            <div class="hp-filter-main uk-flex uk-flex-middle">
                ...
                <div class="hp-filter-dropdown" data-uk-dropdown="{mode:'click', pos:'bottom-left'}">
                    <button class="hp-filter-btn">
                        ...
                        <span id="bar-selected-property">Loại hình</span>
                        <i class="fa fa-chevron-down uk-margin-small-left"></i>
                    </button>
                    <div class="uk-dropdown hp-dropdown-panel">
                        <div class="hp-dropdown-header">Loại bất động sản</div>
                                ...
                                @if (isset($propertyTypes))
                                    @foreach ($propertyTypes as $type)
                                        <label class="hp-selection-item uk-flex uk-flex-middle uk-flex-space-between">
                                            ...
                                            <input type="checkbox" class="bar-sync-input" data-name="catalogue_id[]"
                                                value="{{ $type->id }}"
                                                @if (is_array(request('catalogue_id')) && in_array($type->id, request('catalogue_id'))) checked @endif>
                                        </label>
                                    @endforeach
                                @endif
                            </div>
                ...
                <div class="hp-filter-dropdown" data-uk-dropdown="{mode:'click', pos:'bottom-left'}">
                    <button class="hp-filter-btn">
                        <i class="fa fa-tag"></i>
                        <span id="bar-selected-price">Khoảng giá</span>
                        <i class="fa fa-chevron-down uk-margin-small-left"></i>
                    </button>
                    <div class="uk-dropdown hp-dropdown-panel">
                        ...
                        <div class="hp-dropdown-body">
                            ...
                            <div class="hp-selection-list">
                                @foreach (['0-2' => 'Dưới 2 tỷ', '2-3' => '2 - 3 tỷ', '3-5' => '3 - 5 tỷ', '5-7' => '5 - 7 tỷ', '10-999' => 'Trên 10 tỷ'] as $val => $txt)
                                    <label class="hp-selection-item uk-flex uk-flex-middle uk-flex-space-between">
                                        <span>{{ $txt }}</span>
                                        <input type="radio" class="bar-sync-input" data-name="price"
                                            value="{{ $val }}" {{ request('price') == $val ? 'checked' : '' }}>
                                    </label>
                                @endforeach
                            </div>
                        </div>
                ...
                <div class="hp-filter-dropdown" data-uk-dropdown="{mode:'click', pos:'bottom-left'}">
                    ...
                    <div class="uk-dropdown hp-dropdown-panel">
                        ...
                        <div class="hp-dropdown-body">
                            ...
                            <div class="hp-selection-list">
                                @foreach (['0-30' => 'Dưới 30 m²', '30-50' => '30 - 50 m²', '50-80' => '50 - 80 m²', '80-100' => '80 - 100 m²', '150-9999' => 'Trên 150 m²'] as $val => $txt)
                                    <label class="hp-selection-item uk-flex uk-flex-middle uk-flex-space-between">
                                        <span>{{ $txt }}</span>
                                        <input type="radio" class="bar-sync-input" data-name="area"
                                            value="{{ $val }}"
                                            {{ request('area') == $val ? 'checked' : '' }}>
                                    </label>
                                @endforeach
                            </div>
                        </div>
...
<div id="modal-all-filters" class="uk-modal hp-modal-filter">
    <div class="uk-modal-dialog">
                            ...
                            <div class="hp-btn-toggle-group uk-flex">
                                <label class="uk-width-1-2">
                                    <input type="radio" name="transaction_type" value="74"
                                        ...
                                        {{ request('transaction_type') != '75' ? 'checked' : '' }}
                                        style="display:none">
                                    <span
                                        class="uk-button uk-width-1-1 hp-toggle-btn {{ request('transaction_type') != '75' ? 'active' : '' }}">Tìm
                                        mua</span>
                                </label>
                                <label class="uk-width-1-2">
                            ...
                            <div class="hp-spec-selector uk-flex uk-flex-middle">
                                @foreach (['1', '2', '3', '4', '5+'] as $val)
                                    ...
                                    <label><input type="radio" name="bedrooms" value="{{ $cleanVal }}"
                                            style="display:none" @if (request('bedrooms') == $cleanVal) checked @endif><span
                            ...
                            <div class="hp-spec-selector uk-flex uk-flex-middle">
                                @foreach (['1', '2', '3', '4', '5+'] as $val)
                                    ...
                                    <label><input type="radio" name="bathrooms" value="{{ $cleanVal }}"
                                            style="display:none" @if (request('bathrooms') == $cleanVal) checked @endif><span
                                ...
                                @if (isset($furnitures))
                                    @foreach ($furnitures as $item)
                                        <label class="hp-pill-label">
                                            <input type="checkbox" name="furniture[]" value="{{ $item->id }}"
                                                style="display:none" @if (is_array(request('furniture')) && in_array($item->id, request('furniture'))) checked @endif>
                                            <span
                                    ...
                                    @foreach ($amenities as $item)
                                        <label class="hp-pill-label">
                                            <input type="checkbox" name="amenity[]" value="{{ $item->id }}"
                                                style="display:none" @if (is_array(request('amenity')) && in_array($item->id, request('amenity'))) checked @endif>
                                            <span class="hp-pill">
                                                @if ($item->icon)
                ...
                <div class="hp-selection-list">
                    @if (isset($propertyTypes))
                        @foreach ($propertyTypes as $type)
                            <label class="hp-selection-item uk-flex uk-flex-middle uk-flex-space-between">
                                <span>{{ $type->languages->first()->pivot->name ?? '' }}</span>
                                <input type="checkbox" class="uk-checkbox" name="catalogue_id[]"
        ...
        <div id="hp-view-direction" class="hp-modal-view">
            ...
            <div class="uk-modal-body">
                <div class="hp-selection-list">
                    @php $hDirs = [12=>'Bắc', 13=>'Đông Bắc', 9=>'Đông', 15=>'Đông Nam', 11=>'Nam', 16=>'Tây Nam', 10=>'Tây', 14=>'Tây Bắc']; @endphp
        ...
        <div id="hp-view-balcony" class="hp-modal-view">
            ...
            <div class="uk-modal-body">
                <div class="hp-selection-list">
                    @php $bDirs = [20=>'Bắc', 21=>'Đông Bắc', 17=>'Đông', 23=>'Đông Nam', 19=>'Nam', 24=>'Tây Nam', 18=>'Tây', 22=>'Tây Bắc']; @endphp
        ...
        <div id="hp-view-location" class="hp-modal-view">
            ...
            <div class="uk-modal-body">
                ...
                <div class="gl-form-group">
                    ...
                    <select name="real_estate_catalogue_id" class="gl-select setupSelect2Filter" form="filter-form">
                        <option value="">Tất cả danh mục</option>
                        @if (isset($realEstateCatalogues))
                            @foreach ($realEstateCatalogues as $item)
                                <option value="{{ $item->id }}"
                                    {{ request('real_estate_catalogue_id') == $item->id ? 'selected' : '' }}>
                                    {{ $item->languages->first()->pivot->name ?? '' }}
                                </option>
                            @endforeach
                        @endif
        ...
        <div id="hp-view-price" class="hp-modal-view">
            ...
            <div class="uk-modal-body">
                <div class="hp-custom-range-wrapper uk-margin-bottom">
                    <label class="group-label">Nhập khoảng giá tùy chỉnh (tỷ)</label>
                    <div class="uk-grid uk-grid-small" data-uk-grid-margin>
                        <div class="uk-width-1-2">
                ...
                <div class="hp-selection-list">
                    @foreach (['0-2' => 'Dưới 2 tỷ', '2-3' => '2 - 3 tỷ', '3-5' => '3 - 5 tỷ', '5-7' => '5 - 7 tỷ', '7-10' => '7 - 10 tỷ', '10-999' => 'Trên 10 tỷ'] as $val => $txt)
                        <label class="hp-selection-item uk-flex uk-flex-middle uk-flex-space-between">
                            <span>{{ $txt }}</span>
                            <input type="radio" class="uk-radio" name="price" value="{{ $val }}"
                                form="filter-form" {{ request('price') == $val ? 'checked' : '' }}>
                        </label>
                    @endforeach
                </div>
            </div>
        ...
        <div id="hp-view-area" class="hp-modal-view">
            ...
            <div class="uk-modal-body">
                ...
                <div class="hp-selection-list">
                    @foreach (['0-30' => 'Dưới 30 m²', '30-50' => '30 - 50 m²', '50-80' => '50 - 80 m²', '80-100' => '80 - 100 m²', '100-150' => '100 - 150 m²', '150-9999' => 'Trên 150 m²'] as $val => $txt)
                        <label class="hp-selection-item uk-flex uk-flex-middle uk-flex-space-between">
                            <span>{{ $txt }}</span>
                            <input type="radio" class="uk-radio" name="area" value="{{ $val }}"
                                form="filter-form" {{ request('area') == $val ? 'checked' : '' }}>
                        </label>
                    @endforeach
                </div>
            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        // Shared location state for location.js
        ...
        // Switch Location Mode (After/Before Merger)
        ...
        if (switchMode) {
            switchMode.addEventListener('change', function() {
                ...
                initializeSelect2(); // Re-init for visibility
            });
        }

        // Tab switching logic for filters
        $(document).on('change', '.transaction-radio', function() {
            $('.hp-toggle-btn').removeClass('active');
            $(this).next('.hp-toggle-btn').addClass('active');
        });

        // Bed/Bath selection active state
        ...
        $(document).on('change input', '#modal-all-filters input', function() {
            ...
            updateBarSummaries();
            // updateSummaryTexts is already called by individual listeners in modal
        });

        // Apply from Bar
        $(document).on('click', '.hp-btn-apply-bar', function() {
            $('#filter-form').submit();
        });
        ...
        // Fix background scroll on modal open
        ...
        // Sort functionality
        $(document).on('click', '.hp-sort-list a', function(e) {
            e.preventDefault();
    ...
    function updateBarSummaries() {
        ...
        // Property Type
        const catSelect = modal.querySelector('select[name="real_estate_catalogue_id"]');
        const checkedProps = modal.querySelectorAll('#hp-view-property input:checked');
        const barProp = document.getElementById('bar-selected-property');
        if (barProp) {
            if (catSelect && catSelect.value != "" && catSelect.value != "0") {
                barProp.innerText = catSelect.options[catSelect.selectedIndex].text;
    ...
    function updateSummaryTexts() {
        ...
        // Update Property Type Summary (Catalgoue)
        const catSelect = modal.querySelector('select[name="real_estate_catalogue_id"]');
        const checkedProps = modal.querySelectorAll('#hp-view-property input:checked');
        ...
        if (propEl) {
            if (catSelect && catSelect.value != "" && catSelect.value != "0") {
                propEl.innerText = catSelect.options[catSelect.selectedIndex].text;
        ...
        if (locEl) {
            ...
            if (isAfter) {
                ...
                const w = modal.querySelector('select[name="ward_code"]');
                if (p && p.value != 0 && p.value != "") {
                    label = p.options[p.selectedIndex].text;
                    if (w && w.value != 0 && w.value != "") label += ' - ' + w.options[w.selectedIndex].text;
                }
            } else {
                const p = modal.querySelector('select[name="old_province_code"]');
                const d = modal.querySelector('select[name="old_district_code"]');
                const w = modal.querySelector('select[name="old_ward_code"]');
                if (p && p.value != 0 && p.value != "") {
                    label = p.options[p.selectedIndex].text;
            ...
            if (label !== 'Trên toàn quốc') {
                locEl.classList.remove('uk-text-muted');
            } else {
                locEl.classList.add('uk-text-muted');
            }
        }

        // Update Price Summary
        const priceInput = modal.querySelector('input[name="price"]:checked');
        const priceMinInput = modal.querySelector('input[name="price_min"]');
        const priceMaxInput = modal.querySelector('input[name="price_max"]');
        ...
        if (pEl) {
            ...
            if (priceMin || priceMax) {
                pEl.innerText = `${priceMin || 0} - ${priceMax || '∞'} tỷ`;
        ...
        const areaMinInput = modal.querySelector('input[name="area_min"]');
        const areaMaxInput = modal.querySelector('input[name="area_max"]');
        const aEl = document.getElementById('selected-area-text');
        if (aEl) {
            const areaMin = areaMinInput ? areaMinInput.value : '';
            const areaMax = areaMaxInput ? areaMaxInput.value : '';
            if (areaMin || areaMax) {
        ...
        // Update Direction Summary
        const houseDir = modal.querySelector('input[name="house_direction"]:checked');
        const hEl = document.getElementById('selected-house-direction-text');
        if (hEl) {
            if (houseDir) {
                hEl.innerText = houseDir.closest('label').querySelector('span').innerText;
                hEl.classList.remove('uk-text-muted');
            } else {
                hEl.innerText = 'Tất cả hướng';
                hEl.classList.add('uk-text-muted');
            }
        }

        const balconyDir = modal.querySelector('input[name="balcony_direction"]:checked');
        const bEl = document.getElementById('selected-balcony-direction-text');
        if (bEl) {
            if (balconyDir) {
                bEl.innerText = balconyDir.closest('label').querySelector('span').innerText;
                bEl.classList.remove('uk-text-muted');
            } else {
                bEl.innerText = 'Tất cả hướng';
                bEl.classList.add('uk-text-muted');
            }
        }
    }
</script>
